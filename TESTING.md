...
> **Phase 27 (pipeline hardening, 2026-08-15): the integration suite now runs in CI for the first time, and found a real defect on its first run.** `pnpm test` is unit-only, so 530 integration tests had been gating nothing — they ran on one laptop. CI now does `db:deploy` → `db:seed` → `test:integration`, plus `scripts/verify-migration-repair.sh`. **Eleven payment tests that pass locally failed immediately in CI**: `RATE_PLAN_SEASONAL` (`…000000000501`) is referenced by the payment, invoice and gate-pass suites and was created by **nothing** — not `setup.ts`, not `seed.ts` — surviving only as leftover rows in the single dev database the suite had ever met. On a freshly seeded database every fixture lot was rejected 400. **This is the sharpest available argument for running a suite somewhere other than where it was written**: it was passing partly on undeclared, unreproducible data.

> **Fixture upserts must assert their own shape.** `setup.ts` and `packages/db/prisma/seed.ts` claim the same fixed IDs with *different* values — rate plan `…550` is the suite's unrestricted full-year plan in one and a POTATO-only March–September plan in the other. Every `update: {}` / `update: { isActive: true }` left whichever definition was written first in place, so the suite silently inherited the seed's. Each upsert now writes the same shape on update as on create. A fixture that depends on its row not already existing is fragile by construction.

> **`pnpm build` is part of the gate, not a formality.** CI was red from phase/22 to phase/26 and nobody noticed: a helper exported from a `page.tsx` broke `next build`, which is the only thing that enforces the App Router's export rules — plain `tsc` does not. Four phases were signed off on green tests over a broken build, and it would have made the v0.4.0 release publish nothing. `main` is now protected with `build-and-test` required and `enforce_admins: true`, verified by a rejected direct push.

> **The migration-repair regression reproduces the fault before asserting the fix.** `scripts/verify-migration-repair.sh` builds a pre-rebaseline `_prisma_migrations`, asserts plain `migrate deploy` fails and then stays failed with P3009, and only then asserts `db:deploy` repairs it, adopts the baseline rather than replaying it, leaves the legacy rows untouched, and is a clean no-op on a second run. **It fails loudly if the failure stops reproducing** — a repair test that never sees a broken database would pass just as happily against a no-op, which is exactly how the old advisory-lock tests stayed green for years while acquiring no lock.

> **Phase 26 (update pipeline, 2026-08-13): 219 unit + 530 integration (api, +3 across 39 files) + 131 unit (web, unchanged) green, zero regressions.** The +3 are `chart-of-accounts-sync.integration.test.ts`, covering the step every client box now runs on every update: sync adds only missing accounts, leaves an owner's rename untouched, is a no-op on a second run, and moves 6110 from 6000 to 6900 only from that exact old parent. The rename and the "only from 6000" cases are the ones that matter — they are what fails if someone later "simplifies" the INSERT-only sync into an upsert, or if the re-parent silently no-ops inside its bare `try/catch`.

> **`withGuardsDisabled()` is global DDL, not session state — and three mid-test calls broke two unrelated files.** `financial_guards_set()` runs `ALTER TABLE ... DISABLE/ENABLE TRIGGER` across twelve tables, taking ACCESS EXCLUSIVE locks database-wide. The new sync test initially wrapped each fixture mutation in it; that stalled every other connection in the suite — `opening-balances`' `beforeAll` timed out at 15s (skipping all 15 of its tests, which is what turned 4 skips into 19), an unrelated `outbound` create returned no body, and total runtime nearly doubled (479s → 942s). **Both failing files passed when run in isolation**, so only a full-suite run on the parent branch attributed it correctly: clean `phase/25` measured 527/4/38 with zero failures, `phase/26` with the new file removed measured the same, and `phase/26` with it measured 2 failed files. The calls were never needed — `guard_chart_of_accounts` is BEFORE UPDATE and only locks structural fields once an account carries postings, and fixture accounts carry none. Only append-only `audit_log` cleanup genuinely requires the toggle, and that belongs in `afterAll` where every other file puts it. **The generalisable rule: a suite failure is not "pre-existing" until you have run the full suite on the parent branch — running just the failing files proves nothing about ordering effects.**

> **Not covered by any vitest suite, verified by scratch container instead: the migration-repair path in `packages/db/prisma/deploy.ts`.** It only executes against a `_prisma_migrations` state that cannot be built on the dev box, so it is proven the same way the F-2a installer path is (see the 2026-07-18 entry above): a throwaway `postgres:16-alpine` with the client's exact broken history rebuilt on it. Full transcript in `phases/phase-26-update-pipeline.md` §Verification. **The failure reproduced first, before the fix was run** — attempt 1 `P3018 / 42710 type "UserRole" already exists`, attempt 2 `P3009`, permanently stuck — which is what makes the subsequent green meaningful rather than vacuous. Note the error code: the baseline creates enum types before tables, so the collision is 42710, not the 42P07 predicted from reading the migration file; the code comment was corrected to match what actually happens. `db:deploy` then adopted the baseline, applied all 16 real migrations (20 history rows, 0 unfinished), and was a clean no-op on a second run; against a facility one release behind it added exactly the 3 missing accounts, re-parented 6110, and left an owner's rename untouched. Compose was checked by config rather than by build: `config --services` returns `postgres api web caddy` (migrate is unreachable from `up -d`), `api depends_on` is `["postgres"]`. **Still unexercised: a full image build**, so `update.ps1` end-to-end is reasoned-through, not run.
