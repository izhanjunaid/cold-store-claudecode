...
> **Not covered by any suite: the migration-repair path in `packages/db/prisma/deploy.ts`.** It only executes against a `_prisma_migrations` state that cannot be built on the dev box, and constructing it needs a throwaway Postgres. Verify by scratch container per `phases/phase-26-update-pipeline.md` §Verification before shipping to a client — this is the code that fixes the reported fault, so shipping it unrun would be shipping the fix untested.
