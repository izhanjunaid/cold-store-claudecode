...
| P1-7 | **`number_format` setting is wired to nothing** | **OPEN** — `format.ts:35,56` hardcode `en-PK`; `accounting-format.ts:23,53` hardcode `en-US`; no consumer of the setting exists |
| P1-8 | **No PDF surface formats money** — invoices print `1234567.5` | **OPEN** — `pdf.service.ts:6` registers only the `eq` helper |
...
| P2-1 | **Money inputs are scroll-mutable** — a stray wheel over a focused number input silently changes an amount | **OPEN** — zero `onWheel` handlers in `apps/web/src` |
...
| P2-3 | Account picker is a plain `<select>` over 84 accounts; `ComboboxField` ships and is used elsewhere | **OPEN** — `journal-entries/new/page.tsx:149-151` |
| P2-4 | Voucher running totals bypass the shared formatter | **OPEN** — `journal-entries/new/page.tsx:175,176,179` call bare `.toLocaleString()` |
...
| P2-13 | Statement and operational formatters disagree on the zero/null glyph | **OPEN** — `format.ts:34` returns `—` (U+2014); `accounting-format.ts:20,22` return `–` (U+2013). The comment at `format.ts:6-7` describes the other file incorrectly |
...
| ID | Finding | Status |
|---|---|---|
| P3-1 | Grouped amount input yields a silent zero — typing `40,00,000` posts `0` | **OPEN** — `journal-entries/new/page.tsx:73-74`, `parseFloat(x) \|\| 0` |
