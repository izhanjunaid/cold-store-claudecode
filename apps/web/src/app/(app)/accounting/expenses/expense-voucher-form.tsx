...
export interface ExpenseVoucherFormProps {
  onCreated: (voucher: CreatedVoucher) => void;
  onCancel: () => void;
}

/**
 * Shared by the full-page `/accounting/expenses/new` route (a command-palette
 * deep-link target) and the quick-entry drawer on the expenses list — same
 * fields, same validation, same double-submit guard. The caller decides what
 * "done" means via `onCreated` (navigate vs. close-and-refresh); mounting a
 * fresh instance per attempt (page: navigates away and never returns here;
 * drawer: conditionally rendered, so closing unmounts it) is what makes the
 * double-submit guard's never-reset `submittingRef` safe in both places.
 */
export function ExpenseVoucherForm({ onCreated, onCancel }: ExpenseVoucherFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const created = await apiClient<CreatedVoucher>('/v1/expense-vouchers', {
        method: 'POST',
        body: {
      ...
      onCreated(created);
      // Stay disabled — the page variant navigates away (unmounting this
      // instance) and the drawer variant closes (also unmounting it via its
      // conditional render), so re-arming here would only reopen the
      // duplicate-submit window before either transition completes.
    } catch (err) {
